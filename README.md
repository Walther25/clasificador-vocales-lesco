...
![Comparación de la exactitud de los modelos](reports/graficos/09_comparacion_modelos.png)

*Exactitud de cada modelo en el conjunto de prueba.*

La **matriz de confusión** del mejor modelo (transfer learning) muestra en qué vocales acierta y cuáles a veces confunde:

![Matriz de confusión — Transfer Learning](reports/graficos/08_transfer_matriz_confusion.png)

Las demás gráficas (curvas de entrenamiento y matrices de confusión de cada modelo) están en `reports/graficos/`.
...
![Ejemplos del dataset](reports/graficos/01_muestras_dataset.png)

*Algunas fotos del dataset, ya redimensionadas a 128×128.*
...
│   └── script/              # Versión para ejecutar localmente (.py)
│       ├── preparar_dataset_lesco.py
│       ├── proyecto2_cnn_multiclase.py
│       ├── clasificar_imagenes.py
│       └── requirements.txt
├── DATASET.md
├── MODEL_CARD.md
├── README.md
...
El flujo tiene **tres pasos**, cada uno con su propio archivo. Hay dos versiones del código:

- **`src/notebook/`** — notebooks (`.ipynb`) para **Google Colab**. Es la forma más fácil: es gratis y no requiere instalar nada.
- **`src/script/`** — los mismos programas en `.py` para ejecutar **localmente** (por ejemplo en VS Code). Primero se instalan las dependencias con `pip install -r requirements.txt` y luego se corre cada archivo.
